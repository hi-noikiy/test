...
class Amasty_Fpccrawler_Model_Observer
{
    ...
    public function generateQueue()
    {
        ...
        $lockFile = Mage::getBaseDir('tmp') . DS . $this::FILE_LOCK_GENERATE;
        $fp       = fopen($lockFile, 'w');
        ...
        foreach ($source as $item) {
            ...
            if ($cnt++ > $this::MAX_ROWS_BEFORE_CLEAN) {
                $cnt = 0;
                $resource->cleanQueue();
            }
        }
    ...
    public function processQueue()
    {
        ...
        // check if another cron is still running
        $lockFile = Mage::getBaseDir('tmp') . DS . $this::FILE_LOCK_PROCESS;
        $fp       = fopen($lockFile, 'w');
        if (!flock($fp, LOCK_EX | LOCK_NB)) {
            die('Another FpcCrawler cron job already working');// another lock detected
        }
        ...
        $source  = Mage::getResourceModel('amfpccrawler/queue_collection');
        $source->setOrder('rate', 'DESC')->getSelect()->limit($limit);
        $log->cleanLog();
