...
class Magestore_Onestepcheckout_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function indexAction() {
        ...
        if(!Mage::getSingleton('customer/session')->isLoggedIn()){
            ...
            if($this->getRequest()->getParam('osctype') && $this->getRequest()->getParam('osctype') == 'guest')
            {
            } else {
                    Mage::getSingleton('customer/session')->setBeforeAuthUrl(Mage::helper('core/url')->getCurrentUrl());
                    $this->_redirect('customer/account/login', array('ref' => 'osc'));
                    return;
            }
...
//        $this->loadLayout();
    ...
    public function saveOrderAction() {
      ...
      if (isset($customerAddressId) ) {
             /* Start: Modified by ktpl - Billing Dropdown not show *
          if($customerAddressId != ''){
                $addressData = $this->getRequest()->getPost('billing');
            ...
            //----------------- end Modified Billing Dropdown not show here------------------
