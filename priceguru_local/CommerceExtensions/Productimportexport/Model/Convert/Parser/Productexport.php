...
/**
					...
					/* PRODUCT CATEGORIES EXPORT END */
					 ...
					 /* ADDITIONAL CATEGORY ID EXPORT FOR 1.4 ONLY [END] */
						...
						*/
						...
						$row['associated'] = '';
						if($product->getTypeId() == "configurable") {
							$finalassociatedproducts="";
							$associatedProducts = Mage::getSingleton('catalog/product_type')->factory($product)->getUsedProducts($product);
							foreach($associatedProducts as $associatedProduct) {
								$finalassociatedproducts .= $associatedProduct->getSku() . ",";
							}
							$row['associated'] = substr_replace($finalassociatedproducts,"",-1);
						}
							...
							if($product->getTypeId() == "bundle") {
								...
								foreach($optionModel as $eachOption) {
										//fix here for store view by bundle title.. and the rowCount = 0 is when no value for store set default to admin view
										$selectOptionID = "SELECT title FROM ".$prefix."catalog_product_bundle_option_value WHERE option_id = ".$eachOption->getData('option_id')." AND store_id = ".$product->getStoreId()."";
										$Optiondatarows = $read->query($selectOptionID);
										
										if($Optiondatarows->rowCount()==0) {
											$selectOptionID = "SELECT title FROM ".$prefix."catalog_product_bundle_option_value WHERE option_id = ".$eachOption->getData('option_id')." AND store_id = 0";
											$Optiondatarows = $read->query($selectOptionID);
										}
						...
						if($product->getTypeId() == "grouped") {
							...
							foreach($associatedProducts as $associatedProduct) {
								if($this->getVar('export_grouped_position') == "true") {
									$finalgroupedproducts .= $associatedProduct->getPosition() . ":" . $associatedProduct->getSku() . ":" . $associatedProduct->getQty() . ",";
								} else {
									$finalgroupedproducts .= $associatedProduct->getSku() . ",";
								}
							}
							$row['grouped'] = substr_replace($finalgroupedproducts,"",-1);
						}
    ...
    public function unparse()
    {
	    ...
	    if($this->getVar('export_filter_by_category_ids') != "") {	
		  ...
		  if($this->getVar('filter/qty/from')!="" && $this->getVar('filter/qty/to')!="") {
			#$entityIds_cat_filters->addAttributeToFilter('attribute_set_id',  $this->getVar('filter/attribute_set'));
			#$entityIds_cat_filters->addAttributeToFilter('qty' , array( "from" => $this->getVar('filter/qty/from'), "to" => $this->getVar('filter/qty/to')));
			$entityIds_cat_filters = Mage::getModel('catalog/product')->getCollection()
							->addAttributeToSelect('*')
							->joinField('qty',
		...
		} else if($this->getVar('export_filter_by_skus') != "") {	
		
			$product_skus = explode(",",$this->getVar('export_filter_by_skus'));
			  
			$entityIds = Mage::getModel('catalog/product')->getCollection();
			$entityIds->addAttributeToSelect('*');
			$entityIds->addAttributeToFilter('sku', array( 'in' => array($product_skus) ) );
			...
			if ($overallcount < $recordlimitend && $overallcount >= $recordlimitstart) {
				...
				if($this->getVar('export_multi_store') == "true") {	
					...
					if ($this->getStore()->getCode() == Mage_Core_Model_Store::ADMIN_CODE) {
						...
						//this is for single filter
						/*
						$websiteCode = $this->getStore()->getCode();
