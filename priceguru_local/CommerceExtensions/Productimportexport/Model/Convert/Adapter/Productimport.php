	...
	*/
	public function saveRow( array $importData )
	{
		...
		if($this->getBatchParams('update_products_only') == "true") {
			if($this->getBatchParams('ref_by_product_id') == "true" && isset($importData['product_id'])) {
				$productIdupdate = $importData['product_id'];
			} else {
		...
		if (empty($importData['sku']) && $this->getBatchParams('ref_by_product_id') != "true") {
		...
		if ($productId) {
			...
			if($this->getBatchParams('ref_by_product_id') == "true") {
            ...
            foreach ($this->_requiredFields as $field) {
                ...
                if (!isset($importData[$field]) && $attribute && $attribute->getIsRequired()) {
                    ...
                    Mage::throwException($message);
					Mage::log(sprintf('ERROR: %s', $message), null,'ce_product_import_export_errors.log');
                }
            }
        }
		...
		*/
		...
		if ( isset( $importData['grouped'] ) && $importData['grouped'] != "" ) {
			...
			if ( !empty( $linkIds ) ) {
				#$product -> setGroupedLinkData( $linkIds );
				#keeps existing grouped products and adds new ones to them
				if($this->getBatchParams('append_grouped_products') == "true") { 
					$_existingGrouped="";
					...
					$linkIdsFromCsv = $this -> skusToIds( $_existingGrouped, $product );
					#$finalgroupedimport = array_merge($linkIds, $linkIdsFromCsv);
					$finalgroupedimport = $linkIds + $linkIdsFromCsv;
					$product -> setGroupedLinkData( $finalgroupedimport );
				} else {
					$product -> setGroupedLinkData( $linkIds );
				}
			} 
			...
			foreach ($product->getMediaAttributes() as $mediaAttributeCode => $mediaAttribute) {
				if (isset($importData[$mediaAttributeCode])) {
					...
					if($this->getBatchParams('import_images_by_url') == "true" && $file != "" && $file != "no_selection") { 
						...
						if (!isset($path_parts['extension']) || !in_array(strtolower($path_parts['extension']), $imgExtensions)) {
							Mage::log('SKU: '.$importData["sku"]. ' Invalid image file type', null,'ce_product_import_export_errors.log');
						}
						$file = DS . $path_parts['basename'];
						...
						try {
							...
							$client = new Zend_Http_Client($orgfile);
							if(file_exists($fullpath)) {
								unlink($fullpath);
							}
							if ($client->request('GET')->getStatus() != 404) {
								$rawdata=$client->request('GET')->getBody();
								if(trim($rawdata) !="") {
										$fp = fopen($fullpath,'x');
												fwrite($fp, $rawdata);
												fclose($fp);
								} else {
									throw new Exception('Error: ' . $orgfile .' Returned 0 size');
									Mage::log('Error: ' . $orgfile .' Returned 0 size', null,'ce_product_import_export_errors.log');
								}
							}
							/*
	...
	if(trim($gallery_img) != $importData["small_image"]) 
	{
		if( trim($gallery_img) != $importData["thumbnail"]) 
		{
						...
						if($this->getBatchParams('import_images_by_url') == "true") { 
							...
							$gallery_img = DS . $path_parts['basename'];
							...
							$client = new Zend_Http_Client($fileurlpath);
							if(file_exists($fullpath)) {
								unlink($fullpath);
							}
							if ($client->request('GET')->getStatus() != 404) {
								...
								if(trim($rawdata) !="") {
									...
									throw new Exception('Error: ' . $fileurlpath .' Returned 0 size');
									Mage::log('Error: ' . $fileurlpath .' Returned 0 size', null,'ce_product_import_export_errors.log');
								}
							}
							/*
   ...
   foreach($filearrayforimport as $fileinfo) {
	  ...
	  $next_id = $row['Auto_increment'];
        ...
        if (!$file || !file_exists($file)) {
			...
			Mage::log('Link  file '.$file.' not exists', null,'ce_product_import_export_errors.log');
        }
...
/* END FIX FOR IMAGE LABLES */
   ...
   protected function _addCategories($categories, $store)
    {
		/*
		if($this->getBatchParams('fill_all_related_categories') == "true") {
			$finalgroupcategories="";
			$lastcatused="";
			$individualCategories = explode("/", $categories);
            foreach ($individualCategories as $_individualCategory) {
				$finalgroupcategories .= $lastcatused . "" . $_individualCategory . " , ";
				$lastcatused .= $_individualCategory."/";
			}
			$categories = substr_replace($finalgroupcategories,"",-3);
			#print_r($categories);
		}
		*/
