...
class Belitsoft_Survey_SurveyController extends Mage_Core_Controller_Front_Action
{
    ...
    public function finishAction()
    {
        ...
        $orderid = $this->getRequest()->getPost('default_5');
#		Mage::log(__METHOD__.': next survey question data:'.print_r($post, true));
        ...
        if ($questions) {
            ...
            $this->_saveAnswers($questions, $start_id, $mode,$orderid);
        }
...
#			Mage::log(__METHOD__.': end');
    ...
    protected function _checkRestricted()
    {
        ...
        $restricted = Mage::helper('belitsoft_survey')->checkRestricted($orderid);
        $ordercollection = Mage::getModel('sales/order')->getCollection()
                            ->addFieldToFilter('increment_id',$orderid)
                            ->getFirstItem();
        if(!$ordercollection->getIncrementId() ){$restricted= true;}
     ...
     * @return void
     */
    protected function _saveAnswers($questions, $start_id = null, $mode = Belitsoft_Survey_Model_Session::MODE_NEW,$orderid)
    {
