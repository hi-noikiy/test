...
class Contus_Ordercustomer_Adminhtml_OrdercustomerController extends Mage_Adminhtml_Controller_Action {
        ...
        if (!is_array($ordercustomerIds)) {
            ...
            try {
                foreach ($ordercustomerIds as $ordercustomerId) {
                    ...
                    $checkin = strtotime($orders_list['check_ins']);
                    $checkout = strtotime($orders_list['check_outs']);
                            $Nights = $orders_list['nights'];
                            $Customdescription = $orders_list['customdescription'];
                            $check_in = date('d - M, Y',$checkin);
                            $check_out = date('d - M, Y',$checkout);
                            $product = Mage::getModel('catalog/product')->load($productId);
                            $attVal = $product->getOptions();
                            $bcccustomeremail = $product->getCustomeremail();
                    //currency symbol
  ...
  if($Nights!=0){
        ...
        $mailTemplate->setTemplateSubject('Booking Confirmation');
        $mailTemplate->addBcc($bcccustomeremail);
          $mailTemplate->addBcc('reservation@marideal.mu');
