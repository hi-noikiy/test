...
class Ktpl_Customreport_Adminhtml_CimcreditorderController extends Mage_Adminhtml_Controller_Action {

    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('sales/customreport')
    ...
    public function exportCsvAction() {
        ...
        $content = $this->getLayout()->createBlock('customreport/adminhtml_cimcreditorder_export')
                        ->getCsv();
    ...
    public function updateRowFieldsAction()
    {
        ...
        if ($fieldId) {
            $model = Mage::getModel('customreport/salescimorder')->load($fieldId);
            $model->setDcp($dcp);
            ...
            if($deposit != "" && $deposit > 0) {
                $delorder = Mage::getModel('customreport/salesdeliveryorder')->load($model->getOrderId(), 'order_id');
    ...
    public function createshipmentAction() {
        ...
        if($result['error']) {
            ...
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('customreport')->__('Shipment created successfully.'));
        }
    ...
    public function purchaseorderAction()
    {
        ...
        //$order = Mage::getModel('customreport/salespickuporder')->load($order_id, 'real_order_id');
        $pickuporder = Mage::getModel('customreport/poorder')->load($pickup_id);
        if(!$pickuporder->getData()) {
            $pickuporder = Mage::getModel('customreport/salespickuporder')->load($pickup_id); 
    	...
    	$wholesaler = Mage::getModel('customreport/wholesaler')->load($pickuporder->getWholesalerId());
    	if($wholesaler) {
        ...
        try {
			...
			$mpdf->WriteHTML($processedTemplate);
                        if (!file_exists(Mage::getBaseDir('base').'/var/purchase_order')) {
                            mkdir(Mage::getBaseDir('base').'/var/purchase_order', 0777, true);
                        }
			$fn = Mage::getBaseDir('base').'/var/purchase_order/invoice_pur_'.$pickup_id.'.pdf';
			$mpdf->Output($fn, 'F');
            ...
            $translate->setTranslateInline(true);
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('customreport')->__('Purchase order created successfully.'));
    ...
    public function purchaseorderprintAction()
    {
        ...
        //$order = Mage::getModel('customreport/salespickuporder')->load($order_id, 'real_order_id');
        $order = Mage::getModel('customreport/poorder')->getCollection();
        $order->addFieldToFilter('real_order_id', array('eq' => $order_id));
        if($order->count()) {
            $order = Mage::getModel('customreport/poorder')->getCollection();
            $order->addFieldToFilter('po_id', array('eq' => $pickup_id));
            $order->addFieldToFilter('real_order_id', array('eq' => $order_id));
        } else {
            $order = Mage::getModel('customreport/salespickuporder')->getCollection();
            $order->addFieldToFilter('pickup_id', array('eq' => $pickup_id));
            $order->addFieldToFilter('real_order_id', array('eq' => $order_id));            
        }
        $itemoutput = "";
        ...
        foreach($order as $item) {
        	...
        	$wholesaler = Mage::getModel('customreport/wholesaler')->load($item->getWholesalerId());
        	if($wholesaler) {
        		$wholesaler_name = $wholesaler->getName();
        		$wholesaler_address = $wholesaler->getAddress();
        	} else {
        ...
        try {
            ...
            $result['error'] =  Mage::helper('customreport')->__('Unable to create print copy.');
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('customreport')->__('Unable to create print copy.'));
        }
    ...
    public function salesinvoiceAction() {
        ...
        //if($data['invoice_id']!="") {
        $vatinvoice = Mage::getModel('customreport/salesinvoicevat')->load($order_id, 'order_id');
        ...
        $pickuporder = Mage::getModel('customreport/poorder')->getCollection();
        $pickuporder->addFieldToFilter('real_order_id', array('eq' => $order_id));
                
        if(!$pickuporder->count()) {
            $pickuporder = Mage::getModel('customreport/salespickuporder')->getCollection();
            $pickuporder->addFieldToFilter('real_order_id', array('eq' => $order_id));            
        }
        $order = Mage::getModel('sales/order')->load($order_id);
        ...
        $gift_card = $order->getData('gift_voucher_discount');
        ...
        if($gift_card > 0) {
            $grandtotal -= $gift_card;
            $gift_card = number_format($gift_card,2);
        } else { $gift_card = ""; }
        ...
        try {
            ...
            $mpdf->SetDisplayMode('fullpage');
            $mpdf->WriteHTML($processedTemplate);
            if (!file_exists(Mage::getBaseDir('base').'/var/sale_invoice')) {
                mkdir(Mage::getBaseDir('base').'/var/sale_invoice', 0777, true);
            }
            $fn = Mage::getBaseDir('base').'/var/sale_invoice/invoice_vat_'.$order_id.'.pdf';
            $mpdf->Output($fn, 'F');
            ...
            $transactionalEmail->sendTransactional($template_id, $sender, $email, $emailName, $vars, $storeId);
            $translate->setTranslateInline(true);
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('customreport')->__('Invoice created successfully.'));
            //$url = Mage::helper("adminhtml")->getUrl("adminhtml/sales_order/view/order_id/".$real_id);
            $this->_redirect('adminhtml/sales_order/view', array('order_id' => $order_id));
        } catch(Exception $ex) {
            $result['error'] =  Mage::helper('customreport')->__('Unable to send email.');
    ...
    public function deliverynoteAction() {
        ...
        if($order->getIscimorder()) {
        	$salescimorder = Mage::getModel('customreport/salescimorder')->load($order->getIncrementId(), 'order_id');
        ...
        try {
            ...
            $mpdf->SetDisplayMode('fullpage');
            $mpdf->WriteHTML($processedTemplate);
            if (!file_exists(Mage::getBaseDir('base').'/var/deliverynote')) {
                mkdir(Mage::getBaseDir('base').'/var/deliverynote', 0777, true);
            }
            $fn = Mage::getBaseDir('base').'/var/deliverynote/invoice_delivery_'.$order_id.'.pdf';
            $mpdf->Output($fn, 'F');
            ...
            //print_r($response); exit;
            $translate->setTranslateInline(true);
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('customreport')->__('Delivery note invoice created successfully.'));
            //$url = Mage::helper("adminhtml")->getUrl("adminhtml/sales_order/view/order_id/".$real_id);
            $this->_redirect('adminhtml/sales_order/view', array('order_id' => $order_id));
        } catch(Exception $ex) {
            $result['error'] =  Mage::helper('customreport')->__('Unable to send email.');
