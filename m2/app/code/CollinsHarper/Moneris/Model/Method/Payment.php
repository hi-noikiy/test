 ...
 */
namespace CollinsHarper\Moneris\Model\Method;

use CollinsHarper\Moneris\Helper\Data as chHelper;
use CollinsHarper\Moneris\Model\Transaction;
use CollinsHarper\Moneris\Model\AbstractModel as MonerisConstants;
...
class Payment extends AbstractPayment implements TransparentInterface, ConfigInterface
{
    ...
    public function fetchCryptType(\Magento\Framework\DataObject $payment, $amount)
    {
        ...
        if ($payment->getAdditionalInformation('vault_id') && !$payment->getCcNumber()) {
            ...
            $cryptType = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\VaultPayment')
            ->setPayment($payment)
            ->setAmount($amount)
            ->fetchCryptType();
        } else {
            $this->log(__METHOD__ . 'vbv ');
            $cryptType = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Mpi\Txn')
                ->setPayment($payment)
                ->setAmount($amount)
                ->fetchCryptType();
        }
    ...
    public function cavvContinue(\Magento\Framework\DataObject $payment, $paRes, $md, $order)
    {
        ...
        $mpiResponse = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Mpi\Acs')
            ->setPaRes($paRes)
            ->setMd($md)
            ->setPayment($payment)
            ->post();
    ...
    protected function _cavvAuthorize(\Magento\Framework\DataObject $payment, $md, $cavv)
    {
        ...
        if (!empty($mdArray) && isset($mdArray['pan'])) {
            ...
            $transaction = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\Cavv\PreAuth')
            ->setPayment($payment)
            ->setAmount($amount)
            ->setCavv($cavv);
        } else {
            $transaction = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\Vault\PreAuth')
            ->setPayment($payment)
            ->setAmount($amount)
            ->setCavv($cavv);
        }
    ...
    protected function _cavvPurchase(\Magento\Framework\DataObject $payment, $md, $cavv)
    {
        ...
        if (!empty($mdArray) && isset($mdArray['pan'])) {
            $payment->setCcNumber($mdArray['pan']);
            $transaction = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\Cavv\Purchase')
            ->setPayment($payment)
            ->setAmount($amount)
            ->setCavv($cavv);
        } else {
            $transaction = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\Vault\Purchase')
            ->setPayment($payment)
            ->setAmount($amount)
            ->setCavv($cavv);
        }
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        if ($data->hasData('vault_id')
            ...
            $vault = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Vault')->load($data->getData('vault_id'));
    ...
    public function processTransaction($paymentType = self::AUTHORIZE)
    {
        ...
        if ($isRecurring && !$isCompletion && empty($this->payment->getAdditionalInformation('vault_id'))) {
            $vault = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\Vault')
        ...
        if ($paymentType == self::AUTHORIZE) {
            $transaction = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\PreAuth')
                ->setPayment($this->payment)
            ...
            if ($isCompletion) {
                if ($this->payment->getOrder()->getData('total_paid') > 0) {
                    $isReAuth = true;
                    $transaction = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\ReAuth');
                } else {
                    $transaction = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\Completion');
                }
            } else {
                $transaction = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\Purchase')
                    ->setCryptType($cryptType);
            }
    ...
    public function refund(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        $transaction = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\Refund')
            ->setPayment($payment)
            ->setAmount($amount);
    ...
    public function void(\Magento\Payment\Model\InfoInterface $payment)
    {
        ...
        $transaction = $this->getHelper()->getObject('CollinsHarper\Moneris\Model\Transaction\VoidTransaction')
            ->setPayment($payment)
            ->setAmount(0);
        $result = $transaction->post();
        if (!$result->getSuccess()) {
