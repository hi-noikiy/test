...
class Export
{
    ...
    public function getShippingComments($orderId)
    {
        ...
        $str = implode(',', array_map(function($el){ return $el['comment']; }, $collection->getData()));
        return ltrim($str,',');
    }
    ...
    {   
        ...
        $this->_addFieldToXML("CustomField1", $order->getShippingNotes());
        ...
        $this->_addFieldToXML(
            "InternalNotes",
            '<![CDATA[' . $this->getShippingComments($order->getEntityId()) .']]>'
        );
