...
namespace Customweb\PayEngine3Cw\Block\Adminhtml\Checkout;
...
class Payment extends \Magento\Backend\Block\Widget\Form\Container
{
	 ...
	 * @var string
	 */
	protected $_blockGroup = 'Customweb_PayEngine3Cw';
	...
	protected function _construct()
	{
		...
		$this->setId('payengine3cw_checkout_payment');

		$this->buttonList->update('save', 'label', __('Submit Order'));
		$this->buttonList->update('save', 'class', 'primary');
		$this->buttonList->update('save', 'id', 'payengine3cw-payment-submit-button');
		...
		$this->buttonList->add(
			...
			[
				'label' => __('Cancel'),
				'class' => 'cancel',
				'id' => 'payengine3cw-payment-cancel-button'
			]
		);
	}
	...
	public function getTransaction()
	{
		if (!($this->transaction instanceof \Customweb\PayEngine3Cw\Model\Authorization\Transaction)) {
			...
			if (!($transaction instanceof \Customweb\PayEngine3Cw\Model\Authorization\Transaction) || !$transaction->getId()) {
