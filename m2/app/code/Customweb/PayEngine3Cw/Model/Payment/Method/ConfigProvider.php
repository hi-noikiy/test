...
class ConfigProvider implements \Magento\Checkout\Model\ConfigProviderInterface
{
    ...
    public function __construct(
        ...
        foreach (array_keys($paymentHelper->getPaymentMethods()) as $code) {
        	if (strpos($code, 'payengine3cw_') === 0) {
    ...
    protected function getFailureMessage(AbstractMethod $method)
    {
    	if ($this->_checkoutSession->getQuote()->getPayment()->getMethod() == $method->getCode()) {
	    	$failureMessage = $this->_checkoutSession->getPayEngine3CwFailureMessage();
	    	if (!empty($failureMessage)) {
	    		$this->_checkoutSession->unsPayEngine3CwFailureMessage();
    ...
    protected function getImageUrl(AbstractMethod $method)
    {
    	try {
    		...
    		return $this->_assetRepo->getUrlWithParams('Customweb_PayEngine3Cw/images/payment/method/' . $method->getPaymentMethodName() . '.png', $params);
    ...
    {
    	return new \Customweb\PayEngine3Cw\Model\Renderer\CheckoutForm($method->getCode());
    }
