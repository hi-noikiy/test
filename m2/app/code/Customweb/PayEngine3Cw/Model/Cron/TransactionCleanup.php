...
namespace Customweb\PayEngine3Cw\Model\Cron;
...
class TransactionCleanup
{
	...
	public function execute()
	{
		...
		try {
			$this->_logger->info('PayEngine3Cw: Cleaned up ' . $this->delete($this->getFailedTransactions(), $maxEndtime) . ' failed transactions.');
		} catch (\Exception $e) {
			$this->_logger->error('Error in PayEngine3Cw transaction cleanup cron: ' . $e->getMessage());
		}

		//Remove all pending transaction 6 month after last update
		try {
			$this->_logger->info('PayEngine3Cw: Cleaned up ' . $this->delete($this->getPendingTransactions(), $maxEndtime) . ' pending transactions.');
		} catch (\Exception $e) {
			$this->_logger->error('Error in PayEngine3Cw transaction cleanup cron: ' . $e->getMessage());
		}
		...
		try {
			$this->_logger->info('PayEngine3Cw: Cleaned up ' . $this->delete($this->getNoStatusTransactions(), $maxEndtime) . ' transactions with no status.');
		} catch (\Exception $e) {
			$this->_logger->error('Error in PayEngine3Cw transaction cleanup cron: ' . $e->getMessage());
		}
	}
	...
	/**
	 * @param \Customweb\PayEngine3Cw\Model\Authorization\Transaction[] $items
	...
	/**
	 * @return \Customweb\PayEngine3Cw\Model\ResourceModel\Authorization\Transaction\Collection
	 */
