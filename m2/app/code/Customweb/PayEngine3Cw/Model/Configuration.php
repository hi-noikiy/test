...
class Configuration
{
	...
	{
		$modulesDirectory = $this->_moduleReader->getModuleDir(\Magento\Framework\Module\Dir::MODULE_ETC_DIR, 'Customweb_PayEngine3Cw');
		$this->_xmlParser->load($modulesDirectory . '/config.xml');
		$xmlArray = $this->_xmlParser->xmlToArray();
		foreach ($xmlArray['config']['_value']['default']['payengine3cw']['general'] as $key => $value) {
			if (is_array($value)) {
				$value = $value['_value'];
			}
	...
	{
		$mediaDirectory = $this->_storeManager->getStore()->getBaseMediaDir() . '/customweb/payengine3cw';
		$mediaUrl = $this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA) . '/customweb/payengine3cw';
