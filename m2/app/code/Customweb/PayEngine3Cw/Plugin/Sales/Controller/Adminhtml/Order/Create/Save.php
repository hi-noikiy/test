...
namespace Customweb\PayEngine3Cw\Plugin\Sales\Controller\Adminhtml\Order\Create;
...
class Save
{
	...
	public function aroundExecute(\Magento\Sales\Controller\Adminhtml\Order\Create\Save $subject, \Closure $proceed)
	{
		...
		$this->_coreRegistry->register('payengine3cwcheckout_moto', true);
		$result = $proceed();
		if ($this->_messageManager->getMessages()->getLastAddedMessage() != null
			...
			$order = $this->_coreRegistry->registry('payengine3cw_checkout_last_order');
			if ($order instanceof \Magento\Sales\Model\Order) {
				if ($order->getPayment()->getMethodInstance() instanceof \Customweb\PayEngine3Cw\Model\Payment\Method\AbstractMethod) {
					...
					return $this->_redirectResultFactory->create()->setPath('payengine3cw/checkout/payment', ['order_id' => $order->getId(), 'send_confirmation' => $sendConfirmation]);
