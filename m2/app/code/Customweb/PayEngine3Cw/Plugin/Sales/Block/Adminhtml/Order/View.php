...
namespace Customweb\PayEngine3Cw\Plugin\Sales\Block\Adminhtml\Order;
...
class View
{
	...
	public function aroundGetReviewPaymentUrl(\Magento\Sales\Block\Adminhtml\Order\View $subject, \Closure $proceed, $action)
    {
    	if ($subject->getOrder() instanceof \Magento\Sales\Model\Order
	    		...
	    		&& $subject->getOrder()->getPayment()->getMethodInstance() instanceof \Customweb\PayEngine3Cw\Model\Payment\Method\AbstractMethod) {
		    ...
		    return $subject->getUrl('payengine3cw/transaction/reviewPayment', ['action' => $action, 'transaction_id' => $transaction->getId()]);
    	} else {
    ...
    {
		$this->_coreRegistry->register('payengine3cw_order_view', true, true);
    }
}