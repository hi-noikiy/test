...
define([
	...
	'Customweb_PayEngine3Cw/js/checkout',
	'Customweb_PayEngine3Cw/js/authorizationMethod',
	'Customweb_PayEngine3Cw/js/alias'
], function(
	$,
	Component,
	mageTemplate,
	...
	return Component.extend({
		defaults: {
			template: 'Customweb_PayEngine3Cw/payment/payengine3cw_ratepayinstallments',
			fieldErrorTemplate: '<div for="<%- id %>" generated="true" class="mage-error" id="<%- id %>-error"><%- message %></div>',
			authorizationUrl: url.build('payengine3cw/checkout/authorize/')
		},
		...
		getForm: function() {
			...
			if (!customer.isLoggedIn()) {
				serviceUrl = urlBuilder.createUrl('/guest-carts/:cartId/payengine3cw/checkout/payment-form/:paymentMethod', {
					cartId: quote.getQuoteId(),
					paymentMethod: this.item.method
				});
			} else {
				serviceUrl = urlBuilder.createUrl('/carts/mine/payengine3cw/checkout/payment-form/:paymentMethod', {
					paymentMethod: this.item.method
				});
			}
			storage.get(
