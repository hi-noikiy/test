...
class ShippingTableRates extends Field
{
    ...
    public function render(AbstractElement $element)
    {
        if ($this->manager->isEnabled('Amasty_ShippingTableRates')) {
            ...
            $element->setComment(
                __(
                    'Specify Shipping Table Rates settings properly '
                    . '<a href="%1" target="_blank">See more details here</a>',
                    $url
                )
            );
        } else {
            ...
            $element->setComment(
                __(
                    ' Create an unlimited number of flexible shipping methods with individual rates. '
