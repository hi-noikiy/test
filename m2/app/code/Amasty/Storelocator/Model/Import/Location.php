...
class Location extends AbstractEntity
{
    ...
    protected $_messageTemplates = [
        ValidatorInterface::ERROR_INVALID_PHOTO            => 'Invalid Image or Marker, please check if file exists in the system',
    ...
    public function getUploader()
    {
        if ($this->fileUploader === null) {
            ...
            $tmpPath = $this->mediaDirectory->getAbsolutePath(ImageProcessor::AMLOCATOR_MEDIA_TMP_PATH) . $DS . $this->locationId;
            ...
            $destinationPath = $dirAddon . $DS . $this->mediaDirectory->getRelativePath($destinationDir) . $DS . $this->locationId;
    ...
    public function getCoordinates($address, $rowNum)
    {
        ...
        $url = "https://maps.google.com/maps/api/geocode/json?" . http_build_query($query) . $apiKey;
        ...
        if ($body) {
            ...
            if ($json['status'] == 'OK' && isset($json['results'][0]['geometry']['location'])) {
                ...
                $this->getErrorAggregator()->addError(
                    ...
                    $rowNum,
                    null,
                    __('An error has happened while detecting geo-position data by Google. The response is: ')
                    . $json['error_message']
                );
            }
        }
    ...
    {
        $importParams = $this->getParameters();
        $importPath = $importParams[Import::FIELD_NAME_IMG_FILE_DIR];
        ...
        switch ($key) {
            ...
            case self::COL_LOCATION_GALLERY:
                ...
                foreach (array_filter($fileNames) as $file) {
                    $locationData[$key][] = $this->moveImage($locationData[self::COL_ID], $file, $importPath);
                }
                break;

            case self::COL_MARKER:
                if (!empty($locationData[$key])) {
                    $markerInfo = $this->moveImage($locationData[self::COL_ID], $locationData[$key], $importPath);
