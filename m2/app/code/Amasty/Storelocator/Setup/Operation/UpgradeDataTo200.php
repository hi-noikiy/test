...
class UpgradeDataTo200
{
    ...
    public function execute(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        foreach ($locationsData as $location) {
            ...
            $dataForUpdate = [
                ...
                'meta_description' => substr($this->escaper->escapeHtml($location['description']), 0, self::META_DESCRIPTION_LIMIT),
    ...
    private function needSaveSchedule($scheduleArray)
    {
        foreach ($this->helper->getDaysNames() as $dayKey => $day) {
            if ($scheduleArray[$dayKey][ScheduleDataProvider::OPEN_TIME] !== $this->zeroSchedule
    ...
    private function prepareSchedule($oldSchedule)
    {
        ...
        foreach ($oldSchedule as $dayKey => $schedule) {
            ...
            $newSchedule[$dayKey][ScheduleDataProvider::OPEN_TIME][ScheduleDataProvider::HOURS] = $schedule[ScheduleDataProvider::OPEN_TIME][0];
            $newSchedule[$dayKey][ScheduleDataProvider::OPEN_TIME][ScheduleDataProvider::MINUTES] = $schedule[ScheduleDataProvider::OPEN_TIME][1];
            ...
            $newSchedule[$dayKey][ScheduleDataProvider::CLOSE_TIME][ScheduleDataProvider::HOURS] = $schedule[ScheduleDataProvider::CLOSE_TIME][0];
            $newSchedule[$dayKey][ScheduleDataProvider::CLOSE_TIME][ScheduleDataProvider::MINUTES] = $schedule[ScheduleDataProvider::CLOSE_TIME][1];
        }
