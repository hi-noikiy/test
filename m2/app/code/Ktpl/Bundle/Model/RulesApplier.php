...
class RulesApplier extends \Magento\SalesRule\Model\RulesApplier
{
    ...
    {  
        ...
        $ruleIds = $scopeConfig->getValue('ktpl_bundle_section/bundle/rule_ids', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        //$ruleIdArray = explode(',', $ruleIds);
        $skuIds = $scopeConfig->getValue('ktpl_bundle_section/bundle/sku_ids', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $skuArray = array();
        if(!empty($skuIds)){
        $skuArray = explode(',', $skuIds);
        }
        
        /*Customize Code for Coupon code checking is valid for Prodctsku or not(Bundle product) */ 
            if($rule->getRuleId() == $ruleIds && in_array($item->getSku(),$skuArray)){
                return $this;
            }else{
