...
class AW_Advancedreports_Block_Adminhtml_Setup_Edit_Tabs_General extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        if ($this->getRequest()->getParam('report_id', false) != AW_Advancedreports_Block_Advanced_Abandoned_Grid::GRID_ID_ADVANCED_ABANDONED) {
            ...
            if ($this->isNeedToAddIncludeTaxesOptionToReport($this->getRequest()->getParam('report_id', false))) {
                $this->addIncludeTaxesOption($fieldset);
            }
    ...
    private function isNeedToAddIncludeTaxesOptionToReport($reportId)
    {
        $reportsWithProfits = Mage::helper('advancedreports')->getGridIdsForReportsWithProfits();
        return in_array($reportId, $reportsWithProfits);
    }

    private function addIncludeTaxesOption($fieldset)
    {
        $includeTaxesSource = Mage::getModel('adminhtml/system_config_source_yesno');
        $fieldset->addField(
            'include_taxes_in_profit',
            'ar_select',
            array(
                'label'  => Mage::helper('advancedreports')->__('Include taxes in profit calculation'),
                'title'  => Mage::helper('advancedreports')->__('Include taxes in profit calculation'),
                'name'   => 'include_taxes_in_profit',
                'values' => $includeTaxesSource->toOptionArray(),
            )
        );
    }
}