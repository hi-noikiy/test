...
class AW_Advancedreports_Block_Advanced_Product_Grid extends AW_Advancedreports_Block_Advanced_Grid
{
    ...
    const GRID_ID_ADVANCED_PRODUCT = 'gridProduct';

    public function __construct()
    {
        parent::__construct();
        $this->setId(self::GRID_ID_ADVANCED_PRODUCT);
        $this->setShowAdditionalSelector(true);
    }

    public function getDetailKey()
    {
        $detailKey = $this->_getDefaultDetailKey();
        if ($this->_isDetailKeyValid($this->getFilter('detail_key'))) {
            $detailKey = $this->getFilter('detail_key');
            Mage::helper('advancedreports')->setDetailOption($this->getFilter('detail_key'));
        } else {
            if ($this->_isDetailKeyValid(Mage::helper('advancedreports')->getDetailOption())) {
                $detailKey = Mage::helper('advancedreports')->getDetailOption();
            }
        }
        return $detailKey;
    }

    protected function _getDefaultDetailKey()
    {
        return self::DETAIL_SUMM;
    }

    protected function _isDetailKeyValid($value)
    {
        return array_key_exists($value, $this->_detailOptions);
    }
                ...
                if ($d['period'] == $row['period']) {
                    ...
                    if ($row['column_id']) {
                        ...
                        if ($d['total_revenue'] != 0) {
                            ...
                            $d['total_margin'] =
                                100 * $d['total_profit'] /
                                ($needToIncludeTaxesInProfit ? $d['total_revenue'] : $d['total_revenue_excl_tax']);
                        } else {
                            $d['total_margin'] = 0;
                        }
                    }
    ...
    protected function _getOrderCollection($from, $to)
    {
        ...
        if ($this->_isNeedToAddProfitInfo()) {
    ...
    protected function _prepareColumns()
    {
        ...
        foreach ($this->_skus as $sku) {
            if ((strpos(trim($sku), "*") !== false) && $this->getGrouped()) {
                $this->addColumn(
        ...
        if ($this->_isNeedToAddProfitColumns()) {
    ...
    protected function _isNeedToAddProfitColumns()
    {
        $skus = $this->getSkus();
        $lastSku = array_pop($skus);
        $lastSku = trim($lastSku);
        return (
            count($this->getSkus()) == 1
            && $this->getGrouped()
            && strpos($lastSku, "*") === false
        );
    }

    protected function _prepareColumnsForSetup()
    {
        if ($this->_isNeedToAddProfitColumns()) {
            $this->addProfitColumns();
        }
    ...
    {
        return (
            parent::getCustomColumnConfigEnabled()
            && $this->_isNeedToAddProfitColumns()
        );
    }
}