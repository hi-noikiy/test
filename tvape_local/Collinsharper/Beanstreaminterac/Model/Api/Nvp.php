...
class Collinsharper_Beanstreaminterac_Model_Api_Nvp extends Collinsharper_Beanstreaminterac_Model_Api_Abstract
{
    /**
    ...
    public function getApiEndpoint()
    {
        if (!$this->getData('api_endpoint')) {
            if ($this->getSandboxFlag()) {
                $default = 'https://secure.beanstreaminterac.com/process.cgi';
            } else {
                $default = 'https://secure.beanstreaminterac.com/process.cgi';
            }
    ...
    {
        if (!$this->hasBeanstreaminteracUrl()) {
            if ($this->getSandboxFlag()) {
                $default = 'https://secure.beanstreaminterac.com/process.cgi';
            } else {
                $default = 'https://secure.beanstreaminterac.com/process.cgi';
            ...
            $url = $this->getConfigData('beanstreaminterac_url', $default);
        } else {
            $url = $this->getData('beanstreaminterac_url');
        }
     ...
     * identifies to beanstreaminterac your transaction
     * In this example, when the script sees a token, the script
     * knows that the buyer has already authorized payment through
     * beanstreaminterac.  If no token was found, the action is to send the buyer
     * to beanstreaminterac to first authorize payment
     */
    public function callSetExpressCheckout()
    {
        ...
        //'---------------------------------------------------------------------------------------------------------------
        //' Make the API call to beanstreaminterac
        //' If the API call succeded, then redirect the buyer to beanstreaminterac to begin to authorize payment.
        ...
        $this->setRedirectUrl($this->getBeanstreaminteracUrl());
        return $resArr;
    }
    ...
    */
    function callGetExpressCheckoutDetails()
    {
        ...
        //' at beanstreaminterac.  The function will call beanstreaminterac to obtain the details
        ...
        //'---------------------------------------------------------------------------
        //' Build a second API request to beanstreaminterac, using the token as the
        ...
        $this->setBeanstreaminteracPayerEmail($resArr['EMAIL']);
        ...
        $a->setTelephone(Mage::helper('Beanstreaminterac')->__('N/A'));
    ...
    */
    public function callDoExpressCheckoutPayment()
    {
        /* Gather the information to make the final call to
           finalize the beanstreaminterac payment.  The variable nvpstr
        ...
        $nvpArr = array(
         ...
         /* Make the call to beanstreaminterac to finalize payment
            If an error occured, show the resulting errors
            */
...
#echo "<pre>".print_r($nvpArr,1)."</pre>"; die;
    ...
    /**
     * Function to perform the API call to beanstreaminterac using API signature
     *
    ...
    public function call($methodName, array $nvpArr)
    {
        ...
        if ($this->getDebug()) {
            $debug = Mage::getModel('beanstreaminterac/api_debug')
