...
//        if (!in_array($action, array('delete', 'ajaxDelete', 'loadBlock'))) //loadBlock - admin
//            return;
        ...
        if ( ! $this->_onCollectTotalAfterBusy) {
            ...
            foreach ($observer->getQuote()->getAllItems() as $item)
            {
                if ($item->getIsPromo())
                {
                    ...
                    {
                        $map = unserialize($observer->getQuote()->getGiftMap());
                        if(is_array($map) && count($map) > 0)
                        {
                            foreach ($map as $giftProductId => $data)
                            {
                                if($giftProductId == $item->getProductId())
                                {
                                    if(count($data) == 0)
                                    {
                                        $cart->removeItem($item->getId());
