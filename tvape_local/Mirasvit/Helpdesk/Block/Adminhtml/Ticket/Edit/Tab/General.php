...
class Mirasvit_Helpdesk_Block_Adminhtml_Ticket_Edit_Tab_General extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function getGeneralForm()
    {
        ...
        // Add check for non-department user
        $ownerCode = (int) $ticket->getDepartmentId().'_'.(int) $ticket->getUserId();
        $ownerCodes = Mage::helper('helpdesk')->getAdminOwnerOptionArray(false, $ticket->getStoreId());
        if(!isset($ownerCodes[$ownerCode])) {
            $ownerCode = (int) $ticket->getDepartmentId().'_0';
        }
...
<button type='button' id='find-customer-btn' data-url='".Mage::helper('adminhtml')->getUrl('adminhtml/helpdesk_ticket/customerfind')."'><span>".Mage::helper('helpdesk')->__('Find').'</span></button>
    ...
    protected function getMessageForm()
    {
        ...
        $element->setAfterElementHtml("<div id='reply_note'></div>");

        $fieldset->addField('third_party_email_select', 'select', array(
            'label'    => Mage::helper('helpdesk')->__('Select Third Party Email'),
            'name'     => 'third_party_email_selct',
            'value'    => $ticket->getThirdPartyEmail(),
            'values'   => Mage::getModel('helpdesk/thirdPartyEmail')
                ->getPreparedCollection($ticket->getStoreId())->toOptionArray(true, true),
        ));
        ...
        if ($collection->count()) {
            ...
            foreach ($collection as $template) {
                ...
                $text = htmlspecialchars($text); //protect from XSS
