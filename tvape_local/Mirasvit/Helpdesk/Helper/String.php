...
class Mirasvit_Helpdesk_Helper_String extends Varien_Object
{
    ...
    /**
     * [#a04-l0ngukzifb#]_[#4935869565dc4af08ec6504994e37133#] - example of ebay code
     *
     * @param string $text
     * @return string
     */
    public function parseEbayCodeFromMessage($text)
    {
        $return = '';

        $pattern = '/\D{2}[[:alnum:]]{3}\D[[:alnum:]]{10}\D{5}[[:alnum:]]{32}\D{2}/';
    ...
    public function removeTime($body)
    {
        $lines = explode("\n", $body);
        while (
            count($lines) > 0 && (trim(end($lines)) == '' || trim(end($lines)) == '>' || trim(end($lines)) == '>>')
        ) {
            array_pop($lines);
        }

        $lines = $this->removeGmailQuote($lines);
        ...
        if ((
                $this->validateDate($lastline) || substr($lastline, 0, 2) == 'On' || substr($lastline, 0, 2) == 'El'
                || substr($lastline, 0, 10) == 'Enviado el' || substr($lastline, 0, 2) == 'Em'
            ) && substr($lastline, -1) == ':'
        ) {
            array_pop($lines);
        }
     ...
     * @return array
     */
    public function removeGmailQuote($lines)
    {
        //2014-03-26 19:47 GMT+02:00 Sales
        //<support2@mirasvit.com.ua>:
        if (($length = count($lines)) > 1) {
            $email = $lines[$length-1];
            $date = $lines[$length-2];

            if (substr($email, -1) == ':') {
                $email = trim($email, '<>:');
                if ($this->validateEmail($email) && $this->validateDate($date)) {
                    unset($lines[$length - 1], $lines[$length - 2]);
                }
            }
        }
    ...
    {
        if (preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{1,2}:[0-9]{2} GMT/', $date)) {
            return true;
        }
        ...
        if (class_exists('IntlDateFormatter')) {
            ...
            foreach ($dateFormats as $dateFormat) {
                foreach ($dateFormats as $timeFormat) {
                    $formatter = new IntlDateFormatter("es_ES", $dateFormat, $timeFormat);
