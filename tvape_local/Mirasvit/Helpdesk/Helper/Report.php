...
class Mirasvit_Helpdesk_Helper_Report extends Mage_Core_Helper_Abstract
{
    ...
    public function periodCallback($value, $row, $column)
    {
        ...
        /*Note: the hour_of_day case uses Mage::timestamp because the $value provided includes time information.
        The other cases use strtotime because they do not provide time information, so correcting
        for local timezones causes problems.*/
        switch ($column) {
            case 'month':
            ...
            case 'hour_of_day':
                $value = date('h:00 A',  Mage::getModel('core/date')->timestamp($value));
                break;
