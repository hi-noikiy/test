...
class Mirasvit_Helpdesk_Model_Rule_Condition_Ticket extends Mage_Rule_Model_Condition_Abstract
{
    public function loadAttributeOptions()
    {
        $attributes = array(
            ...
            'last_message_type' => Mage::helper('helpdesk')->__('Last message type'),
            ...
            'minutes_since_created_at' => Mage::helper('helpdesk')->__('Minutes since Created'),
            'minutes_since_updated_at' => Mage::helper('helpdesk')->__('Minutes since Updated'),
            'minutes_since_last_reply_at' => Mage::helper('helpdesk')->__('Minutes since Last reply'),
    ...
    public function validate(Varien_Object $object)
    {
        ...
        } elseif ($attrCode == 'last_message_type') {
            $lastMessage = $object->getLastMessage();
        ...
        } elseif (strpos($attrCode, 'minutes_since_') === 0) {
            $attrCode = str_replace('minutes_since_', '', $attrCode);
            if ($timestamp = $object->getData($attrCode)) {
                $diff = abs(strtotime(Mage::getModel('core/date')->gmtDate()) - strtotime($timestamp));
            } else {
                $diff = abs(strtotime(Mage::getModel('core/date')->gmtDate()) - strtotime($object->getUpdatedAt()));
        ...
        if ($field && $field->getType() == 'select') {
            ...
            switch ($this->getAttribute()) {
                ...
                case 'last_message_type':
                    $selectOptions = array(
                        Mirasvit_Helpdesk_Model_Config::MESSAGE_PUBLIC => Mage::helper('helpdesk')->__('Public'),
                        Mirasvit_Helpdesk_Model_Config::MESSAGE_INTERNAL => Mage::helper('helpdesk')->__('Internal'),
                        Mirasvit_Helpdesk_Model_Config::MESSAGE_PUBLIC_THIRD =>
                            Mage::helper('helpdesk')->__('Third-party public'),
                        Mirasvit_Helpdesk_Model_Config::MESSAGE_INTERNAL_THIRD =>
                            Mage::helper('helpdesk')->__('Third-party private'),
                    );
                    $addNotEmpty = false;
                    break;
                case 'channel':
                    $selectOptions = array(
                        Mirasvit_Helpdesk_Model_Config::CHANNEL_EMAIL => Mage::helper('helpdesk')->__('Email'),
                        Mirasvit_Helpdesk_Model_Config::CHANNEL_BACKEND => Mage::helper('helpdesk')->__('Backend'),
                        Mirasvit_Helpdesk_Model_Config::CHANNEL_FEEDBACK_TAB =>
                            Mage::helper('helpdesk')->__('Contact Tab'),
                        Mirasvit_Helpdesk_Model_Config::CHANNEL_CONTACT_FORM =>
                            Mage::helper('helpdesk')->__('Contact Form'),
                        Mirasvit_Helpdesk_Model_Config::CHANNEL_CUSTOMER_ACCOUNT =>
                            Mage::helper('helpdesk')->__('Customer Account'),
                    );
                    $addNotEmpty = false;
                    break;
                default:
