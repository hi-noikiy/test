...
class Mirasvit_Rma_Block_Adminhtml_Template_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('edit_fieldset', array('legend' =>
            Mage::helper('rma')->__('General Information')));
        if ($template->getId()) {
        ...
        if (Mage::getSingleton('rma/config')->getGeneralIsWysiwygEnabled()) {
            ...
            $fieldset->addField('template', 'textarea', array(
                ...
                'name' => 'template',
                'value' => $template->getTemplate(),
                'note' => Mage::helper('rma')->__('You can use variables: [rma_increment_id], [rma_firstname], ' .
                    '[rma_lastname], [rma_email], [store_name], [store_phone], [store_address], [user_firstname], ' .
