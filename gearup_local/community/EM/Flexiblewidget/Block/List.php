...
implements Mage_Widget_Block_Interface
{
    ...
    protected function _construct()
    {
        ...
        $this->addData(array(
            'cache_lifetime' => 3600,
            'cache_tags'     => array(Mage_Core_Block_Template::CACHE_GROUP, Mage_Catalog_Model_Product::CACHE_TAG)
        ));
    }
    ...
    public function getCacheKeyInfo()
    {
        ...
        $info[] = 'FLEXIBLE_WIDGET';
        $info[] = Mage::getSingleton('customer/session')->getCustomerGroupId();
        $info[] = Mage::app()->getStore()->getCurrentCurrencyCode();
        $info[] = $this->getRequest()->isSecure() ? 'secured' : 'not-secured';
        $info[] = $this->getData('category');
        $info[] = $this->getData('column_count');
        $info[] = $this->getData('limit_count');
        $info[] = $this->getData('order_by');
        $info[] = $this->getData('custom_theme');
        $info[] = $this->getData('sku');
        $info[] = $this->getData('skus');
	...
	public function getLoadedProductCollection()
    {
        ...
        // If SKUs are forced:
        if ($this->hasData('skus')) {
            $collection = $this->loadForcedProductsBySKUs();
        }

        return $collection;
	}

    protected function loadForcedProductsBySKUs()
    {
        $skus = explode(',', preg_replace('/\s+/', '', $this->getSkus()));
        ...
        $allIds = array_keys($this->_productCollection->toArray());
        $limit = (int)$this->getLimitCount();

        $products = Mage::getModel('catalog/product')->getCollection();
        Mage::getModel('catalog/layer')->prepareProductCollection($products);
        $products->addAttributeToSelect(array('description', 'short_description'));
        $products->addFieldToFilter('sku', array('in' => $skus));
        ...
        for ($i=0; $i<$limit-$size; $i++) {
            if (count($allIds)) {
                ...
                $collection->addItem($this->_productCollection->getItemById($id));
            }
        }
    ...
    {
        // If one product selected by SKU:
        ...
        if($config1)
        {
			...
			$products->getSelect()->joinInner(
                array('cis' => $products->getTable('cataloginventory/stock_item')), 'e.entity_id = cis.product_id',
                array('qty'=>'qty')
			);
        ...
        $products->getSelect()->where('cis.qty > ?', 0);
