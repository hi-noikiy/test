...
class MageWorx_SeoReports_Adminhtml_Mageworx_Seoreports_CmsController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function runGenerateAction()
    {
        ...
        switch ($action) {
            case 'start':
                ...
                $tableName   = $tablePrefix . $this->_getReportTableName();
    ...
    protected function _calculateCms($storeId)
    {
        ...
        $sql = "UPDATE `" . $tableName . "` AS srp,
                    (SELECT `prepared_heading`, `store_id`, COUNT(*) AS dupl_count FROM `" . $tableName . "` WHERE `store_id`=" . intval($storeId) . " AND `prepared_heading`!='' GROUP BY `prepared_heading`) AS srpr
    ...
    {
        /** @var MageWorx_SeoReports_Model_Resource_Cms $resource */
        $resource = Mage::getResourceSingleton('mageworx_seoreports/cms');
