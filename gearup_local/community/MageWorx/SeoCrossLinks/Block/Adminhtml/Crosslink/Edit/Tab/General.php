...
class MageWorx_SeoCrossLinks_Block_Adminhtml_Crosslink_Edit_Tab_General extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        /** @var MageWorx_SeoCrossLinks_Helper_Data $helper */
        $helper = Mage::helper('mageworx_seocrosslinks');
        $https = 'http[s]://';
        $customerAcc = 'customer/account/';
        $staticUrlNote0 = $helper->__(
            "Link without '%s' as %s<br>will be converted to<br>%s(store_URL_here)/%s",
            $https,
            $customerAcc,
            $https,
            $customerAcc
        );
        $staticUrlNote1 = $helper->__("Link with '%s' will be added as it is.", $https);
        ...
        if(!empty($data['ref_product_sku']) && !$this->_getIsProductExistBySku($data['ref_product_sku'])){

            $productReferenceNote = $helper->__(
                '<font color="red">Product (SKU \'%s\') not found.</font>',
                $data['ref_product_sku']
            );
        }
        ...
        $fieldset->addField(
            ...
            'select',
            array(
                'label'  => Mage::helper('mageworx_seocrosslinks')->__('Nofollow'),
                'title'  => Mage::helper('mageworx_seocrosslinks')->__('Nofollow'),
                'name'   => 'nofollow_rel',
                'index'  => 'nofollow_rel',
                'values' => Mage::getSingleton('adminhtml/system_config_source_yesno')->toOptionArray()
            )
    ...
    protected function _getDefaultData()
    {
        ...
        return array(
            ...
            'nofollow_rel'      => $helperData->getDefaultNofollow(),
        );
    }

    /**
     * Get the result of the product existence by SKU
     *
     * @param string $sku
     * @return bool
     */
    protected function _getIsProductExistBySku($sku)
    {
