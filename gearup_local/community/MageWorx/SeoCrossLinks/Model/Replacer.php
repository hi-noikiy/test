...
class MageWorx_SeoCrossLinks_Model_Replacer
{
     ...
     * @var array
     */
    protected $_exceptTagsForCrossLinks = array(
        '[not(ancestor::script)]',
        '[not(ancestor::a)]',
        '[not(ancestor::h1)]',
        '[not(ancestor::h2)]',
    ...
    public function replace($collection, $html, $maxReplaceCount, $ignoreProductSku = null, $ignoreCategoryId = null)
    {
        ...
        $expression = $this->_prepareStringExсeptionTags();
        $domNodeList = $xpath->query($expression);
    ...
    /**
     * Combine exception tags to result string
     *
     * @return string
     */
    protected function _prepareStringExсeptionTags()
    {
        $exceptTags   = $this->_exceptTagsForCrossLinks;
        $strexcepTags = implode(" ", $exceptTags);
        $strexcepTags = '//text()'.$strexcepTags;
        return $strexcepTags;
    }
    ...
    protected function _getHtmlHref($crosslink, $keyword, $urlRaw, $name)
    {
        ...
        return "<a class=\"crosslink\" href=\"" . $url . "\" target=\"" . $target . "\" alt=\"" . $title . "\" title=\"" . $title . "\" $rel >" . $keyword . "</a>";
    }
