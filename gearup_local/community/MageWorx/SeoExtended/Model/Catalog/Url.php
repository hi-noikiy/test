...
class MageWorx_SeoExtended_Model_Catalog_Url extends Mage_Catalog_Model_Url
{
    ...
    protected function _getProductRequestPathBefore15ver($product, $category)
    {
        ...
        if ($category->getLevel() > 1) {
            ...
            /** @var Mage_Catalog_Helper_Category $categoryHelper */
            $categoryHelper = Mage::helper('catalog/category');
            $categoryUrl = $categoryHelper->getCategoryUrlPath($category->getUrlPath(), false, $storeId, true);
    ...
    protected function _getProductRequestPathSince17ver($product, $category)
    {
        ...
        else
        {
            ...
            if (strlen($requestPath) > self::ALLOWED_REQUEST_PATH_OVERFLOW + self::MAX_REQUEST_PATH_LENGTH) {
                $requestPath = substr($requestPath, 0, self::MAX_REQUEST_PATH_LENGTH);
            }
