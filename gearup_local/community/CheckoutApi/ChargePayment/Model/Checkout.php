...
abstract class CheckoutApi_ChargePayment_Model_Checkout extends Mage_Payment_Model_Method_Cc
{
    ...
    protected function _getSecretKey($storeId = NULL) {
        ...
        if($this->getCurrencyCode() == 'AED'){ // Need to change currency to AED
            $secretKey = Mage::helper('chargepayment')->getConfigData($this->_code, 'secretkeyAed', $storeId);
        } else {
            $secretKey = Mage::helper('chargepayment')->getConfigData($this->_code, 'secretkeyOther', $storeId);
        }
