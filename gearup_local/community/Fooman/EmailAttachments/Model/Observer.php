...
class Fooman_EmailAttachments_Model_Observer
{
    ...
    public function beforeSendInvoice($observer)
    {
        ...
        $bankPaymentAttachment = Mage::getStoreConfig('sales_email/order/bankpaymentatt', $invoice->getStoreId());

        if ($bankPaymentAttachment) {
            $mailTemplate = Mage::helper('emailattachments')->addFileBankAttachment($bankPaymentAttachment, $mailTemplate);
        }
    }
