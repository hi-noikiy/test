...
class Belvg_Countdown_Model_Observer_Attributes
{
    /**
     ...
     * @return boolean
     */
    public function saveData($productId, $additionally)
    {
        $info                  = array();
    ...
    {
        if (Mage::helper('countdown')->isEnabled()) {
            $countDownChange = $this->_getRequest()->getParam('countdown_change', 0);
            $productIds      = explode(',', $this->_getRequest()->getParam('product_ids'));
            if ($countDownChange && count($productIds) && isset($productIds[0]) && $productIds[0]) {
                foreach ($productIds AS $productId) {
                    $this->saveData($productId, TRUE);
                }
            }
