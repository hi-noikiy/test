...
class AW_Mobile3_Model_Observer
{
    ...
    const HOME_PAGE_REGISTRY_KEY = "is_home_page_flag";
    const HOME_PAGE_RENDERED_REGISTRY_KEY = "is_home_page_rendered_flag";
    const HOME_PAGE_DEFAULT_INDEX_ACTION_REGISTRY_KEY = "is_default_index_action_flag";
    ...
    public function layoutRenderBeforeCmsIndex()
    {
        if (Mage::registry(self::HOME_PAGE_REGISTRY_KEY)) {
            if (Mage::registry(self::HOME_PAGE_RENDERED_REGISTRY_KEY)) {
                $this->_disableRenderingPageNativeBlocks();
            } else {
                Mage::register(self::HOME_PAGE_RENDERED_REGISTRY_KEY, true);
            }
        }
        return $this;
    }
    ...
    public function beforeRenderLayout()
    {
        ...
        if (!$request->getParam(self::TRIGGER_REQUEST_PARAM, false)) {
            if (Mage::getModel('core/store')->isCurrentlySecure() && $this->_isAjax()) {
                ...
                if ((strpos($body, self::REDIRECT_URL_PARAM)) !== false) {
                    $this->_disableRenderingPageNativeBlocks();
                }
            }
            return $this;
        }
        $request->setParam(self::TRIGGER_REQUEST_PARAM,"");
        $requestUri = $_SERVER['REQUEST_URI'];
    ...
    protected function _checkHomePage()
    {
        ...
        if (
            $fullActionName == 'cms/index/defaultIndex'
            && !Mage::registry(self::HOME_PAGE_DEFAULT_INDEX_ACTION_REGISTRY_KEY)
        ) {
        ...
        Mage::register(self::HOME_PAGE_REGISTRY_KEY, true);
        Mage::getSingleton('cms/page')->unsetData();
        if (!Mage::helper('cms/page')->renderPage($controllerAction, $mobileHomePageId)) {
            Mage::register(self::HOME_PAGE_DEFAULT_INDEX_ACTION_REGISTRY_KEY, true);
    ...
    protected function _sendRedirectResponse($url)
    {
        if (Mage::getModel('core/store')->isCurrentlySecure() && $this->_isAjax()) {
            ...
            $responseObject->setData(self::REDIRECT_URL_PARAM, $url);
            $json = $responseObject->toJson();

            $this->_sendResponse($json);
        } else {
            $response = Mage::app()->getFrontController()->getResponse();
