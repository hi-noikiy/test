...
class TM_ProLabels_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function isMobileMode()
    {
        if ($this->_is_mobile === NULL) {
            $mobileDetect = Mage::helper('prolabels/device')->getMobileDetect();
            $this->_is_mobile = $mobileDetect->isMobile() && !$mobileDetect->isTablet();
        }
        ...
        foreach (current($vars) as $var) {
            ...
            if ($var == '#discount_percent#') {
                if ($object->getData('type_id') === 'bundle') {
                    ...
                    $calculatedPrice = $this->calculateProductSpecialPrice($object);
                    $calculatedPrice = $this->Taxprice($calculatedPrice);
                    $regprice = $this->Taxprice($object->getData('price'));
                    $data[$var] = (100 - $calculatedPrice * 100 / $regprice) / $label[$mode . '_round'];
    ...
    public function Taxprice($price = 0){
        $cur = Mage::app()->getStore()->getCurrentCurrencyCode();
        $currencyObj = new Mage_Directory_Model_Currency;
        $currencyObj->setCurrencyCode($cur);
        $taxPrice = Mage::helper('directory')->currencyConvert($price,'USD',$currencyObj);
        if($cur == 'AED' || $cur == 'SAR'){
            $taxPrice = ceil($taxPrice+($taxPrice*5)/100);
        }
        return $taxPrice;
    }
    ...
    public function getCategoryProductUrl($product, $mode, $data)
    {
        ...
        $productName = $this->quoteEscape($productName);
        $style = "style='width:auto;height:auto;'";
        $text = '';
        return "<a href='{$url}' title='{$productName}' {$style}>$text";
    }

    public function _getProductUrl(Mage_Catalog_Model_Product $product, $imgPath, $mode, $data)
    {
    ...
    public function _getTableMargins($position, $imagePath)
    {
    ...
    public function _getTableSize($imagePath)
    {
        if (null === ($image = $this->_getImage($imagePath))) {
            return '';
        }
