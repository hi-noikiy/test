<?php
class Oye_SocialAuth_Model_Twitter_Client
{
    const REDIRECT_URI_ROUTE = 'socialauth/twitter/connect';
    const REQUEST_TOKEN_URI_ROUTE = 'socialauth/twitter/request';

    const OAUTH_URI = 'https://api.twitter.com/oauth';
    const OAUTH2_SERVICE_URI = 'https://api.twitter.com/1.1';    

    const XML_PATH_ENABLED = 'socialauth/oye_socialauth_twitter/enabled';
    const XML_PATH_CLIENT_ID = 'socialauth/oye_socialauth_twitter/client_id';
    const XML_PATH_CLIENT_SECRET = 'socialauth/oye_socialauth_twitter/client_secret';
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401 || $status == 429)) {
                if(isset($decoded_response->error->message)) {
                    ...
                    $message = Mage::helper('oye_socialauth')
                        ->__('Unspecified OAuth error occurred.');
                }

                throw new Oye_SocialAuth_TwitterOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('oye_socialauth')
...
class Oye_SocialAuth_TwitterOAuthException extends Exception
{}
