...
class Magestore_Sociallogin_AmazonloginController extends Mage_Core_Controller_Front_Action
{
    ...
    public function loginAction() {
        ...
        if (!$token) {
            ...
            $this->getResponse()->setBody("<script type=\"text/javascript\">try{window.location.reload(true);}catch(e){window.location.href=\"" . Mage::app()->getStore()->getBaseUrl() . "\"}</script>");
        }
        // get profile
        ...
        if ($profile && $profile->user_id) {
            ...
            if ($data['email']) {
                ...
                $this->getResponse()->setBody("<script type=\"text/javascript\">try{window.location.href=\"" . $this->_loginPostRedirect() . "\";}catch(e){window.location.reload(true);}</script>");
            } else {
                $message = $this->__('You provided a email invalid!');
                Mage::getSingleton('core/session')->addError($message);
                $this->getResponse()->setBody("<script type=\"text/javascript\">try{window.location.reload(true);}catch(e){window.location.href=\"" . Mage::app()->getStore()->getBaseUrl() . "\"}</script>");
