...
class Amasty_Sorting_Block_Adminhtml_System_Config_Fieldset_Info
    extends Mage_Adminhtml_Block_System_Config_Form_Fieldset
{
    protected $_contentHtml = '';

    protected $_moduleCode = 'Amasty_Sorting';

    protected $_userGuideLink = 'https://amasty.com/docs/doku.php?id=magento_1:improved-sorting';
    ...
    public function getAdditionalModuleContent()
    {
        ...
        if (Mage::helper('core')->isModuleEnabled('Mana_Seo')) {
            $messages['notice'] = $this->__('Please make sure all sorting options ('
                . implode(',', array_keys(Mage::helper('amsorting')->getMethodModels()))
                . ') are added to the MANADEV->General->Search Engine Optimization->Additional Sort By Options');
        }

        if (!@class_exists('Amasty_Base_Helper_String')) {
            $messages['error'] = Mage::helper('amsorting')->getUnserializeError();
        }
