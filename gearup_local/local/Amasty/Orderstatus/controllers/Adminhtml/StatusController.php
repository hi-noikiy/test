...
class Amasty_Orderstatus_Adminhtml_StatusController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function deleteAction()
    {
        if ($id = $this->getRequest()->getParam('id')) {
            try {
                ...
                $model->setId($id);
                $model->delete();
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('The status has been deleted.'));
                $this->_redirect('*/*/');
                return;
            }
            catch (Exception $e) {
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Unable to find status to delete.'));
        $this->_redirect('*/*/');
    }
    ...
    public function systemSaveAction()
    {
        ...
        if (is_array($statuses) && !empty($statuses))
        {
            ...
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('The statuses have beed saved.'));
            $this->_redirect('*/*/system');
            return;
        }
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Unable to find statuses to save.'));
        $this->_redirect('*/*/system');
    }
}