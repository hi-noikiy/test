...
class Amasty_Xnotif_Model_Observer extends Mage_ProductAlert_Model_Observer
{
                        ...
                        if ($this->getIsInStock($product)) {
                            if ($alert->getParentId() && !$product->isConfigurable()) {
                                ...
                                if ($parentProduct->isConfigurable()) {
                                    $productUrl .= $this->getConfigurableHash($product, $parentProduct);
                                }
                                $product->setData(
                                    'url',
                                    $productUrl
     ...
     * @return string
     */
    protected function getConfigurableHash($simple, $configurableProduct)
    {
        $hash = '';
        $options = Mage::getModel('catalog/product_type_configurable')->
            getConfigurableAttributes($configurableProduct);

        if ($options->getSize()) {
            foreach ($options as $option) {
                $hash .= '&' . $option->getAttributeId() .
                    '=' . $simple->getData($option->getProductAttribute()->getAttributeCode());
            }
