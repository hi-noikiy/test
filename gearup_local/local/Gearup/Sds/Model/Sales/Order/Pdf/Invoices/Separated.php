...
class Gearup_Sds_Model_Sales_Order_Pdf_Invoices_Separated extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Separated
{
	   ...
	   $shelving4_attribute = null;
        ...
        foreach ($orders as $orderSingle) {
            ...
            foreach ($itemsCollection as $item) {
                ...
                //custom attribute 1
                if ($shelving_yn == 1) {
                    if ($shelving_attribute == 'same_day_shipping') {
                        $searchs = Mage::helper('gearup_sds')->getHorder($product, $orderSingle);
                        if (count($searchs) && $product->getDxbs()) {
                            foreach ($searchs as $search) {
                                $sku_shelving[$sku] = $search['sds'] ? 'Yes' : 'No';
                            }
                        } else {
                ...
                //custom attribute 2
                if ($shelving2_yn == 1) {
                    if ($shelving2_attribute == 'same_day_shipping') {
                        $searchs = Mage::helper('gearup_sds')->getHorder($product, $orderSingle);
                        if (count($searchs) && $product->getDxbs()) {
                            foreach ($searchs as $search) {
                                $sku_shelving2[$sku] = $search['sds'] ? 'Yes' : 'No';
                            }
                        } else {
                ...
                //custom attribute 3
                if ($shelving3_yn == 1) {
                    if ($shelving3_attribute == 'same_day_shipping') {
                        $searchs = Mage::helper('gearup_sds')->getHorder($product, $orderSingle);
                        if (count($searchs) && $product->getDxbs()) {
                            foreach ($searchs as $search) {
                                $sku_shelving3[$sku] = $search['sds'] ? 'Yes' : 'No';
                            }
                        } else {
				...
				//custom attribute 4
                if ($shelving4_yn == 1) {
                    if ($shelving4_attribute == 'same_day_shipping') {
                        $searchs = Mage::helper('gearup_sds')->getHorder($product, $orderSingle);
                        if (count($searchs) && $product->getDxbs()) {
                            foreach ($searchs as $search) {
                                $sku_shelving4[$sku] = $search['sds'] ? 'Yes' : 'No';
                            }
                        } else {
