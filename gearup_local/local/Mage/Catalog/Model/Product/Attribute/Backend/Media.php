...
class Mage_Catalog_Model_Product_Attribute_Backend_Media extends Mage_Eav_Model_Entity_Attribute_Backend_Abstract
{
    ...
    public function afterLoad($object)
    {
        ...
        $value['images'] = array();
        $value['values'] = array();
        $localAttributes = array('label', 'position', 'disabled','survive');
    ...
    public function afterSave($object)
    {
        ...
        foreach ($value['images'] as &$image) {
            ...
            $data['survive'] = (int) $image['survive'];
    ...
    public function addImage(Mage_Catalog_Model_Product $product, $file,
        $mediaAttribute = null, $move = false, $exclude = true,$survive = true)
    {
        $file = realpath($file);
    ...
    public function addImagesWithDifferentMediaAttributes(Mage_Catalog_Model_Product $product,
        $fileAndAttributesArray, $filePath = '', $move = false, $exclude = true, $survive = true) {
        ...
        foreach ($fileAndAttributesArray as $key => $value) {
            ...
            if ($keyInAddedFiles === false) {
                $savedFileName = $this->addImage($product, $filePath . $value['file'], null, $move, $exclude,$survive);
