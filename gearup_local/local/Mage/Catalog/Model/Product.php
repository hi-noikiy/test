...
//        return $result;
    ...
    protected function _beforeSave()
    {
        ...
        if ($this->getDxbs() && Mage::app()->getRequest()->getControllerName() == 'catalog_product') {
            $history = Mage::getModel('gearup_sds/history')->getCollection();
            $history->addFieldToFilter('product_id', array('eq'=>$this->getId()));
            $history->addFieldToFilter('actions', array('like'=>'%firstly added to the DXBS%'));
            //$history->printLogQuery(true,true);
            if (!$history->getSize()) {
                if ($product->getId() && !$product->getData('dxbs') && $this->getData('dxbs')) {
                    $action = 'The Product was firstly added to the DXBS';
                    Mage::helper('gearup_sds')->recordHistory($this->getId(), $action, $qty, $qty);
                }
            }

            $previousSds = Mage::helper('gearup_sds')->sdsStatus($product->getData('same_day_shipping'));

            Mage::getSingleton('core/session')->setPsds($previousSds);
            //var_dump(Mage::getSingleton('core/session')->getPsds());
        }

        if (!$this->getDxbs() && Mage::app()->getRequest()->getControllerName() == 'catalog_product') {
            if ($product->getData('dxbs')) {
                $action = 'Removed Product from the DXBS';
                Mage::helper('gearup_sds')->recordHistory($this->getId(), $action, $qty, $qty);
            }
        }
    ...
    protected function _afterSave()
    {
        ...
        $this->getOptionInstance()->setProduct($this)
            ->saveOptions();
        if ($this->getDxbs() && Mage::app()->getRequest()->getControllerName() == 'catalog_product') {
            $previousSds = Mage::getSingleton('core/session')->getPsds();
            if ($this->getData('is_in_stock')) {
                $previousStockStatus = 'In Stock';
            } else {
                $previousStockStatus = 'Out of Stock';
            }
            if ($this->getStockData('qty')) {
                //$this->setData('same_day_shipping', 1);
                $nowstatus = $this->getStockData('qty') ? 'In Stock' : 'Out of Stock';
                $nowSds = Mage::helper('gearup_sds')->sdsStatus($this->getData('same_day_shipping'));
                //Mage::helper('gearup_sds')->assignSDS($this->getId());
                if ($this->getData('stock_item')) {
                    $action = '"' . $this->getStockData('original_inventory_qty') . ' ' . $previousStockStatus . '; SDS is ' . $previousSds . '", Update QTY to ' . $this->getStockData('qty') . ', "' . $this->getStockData('qty') . ' ' . $nowstatus . '; SDS is  ' . $nowSds . '" - Admin edit Product detail page changed';
                } else {
                    $action = '"New Product", Update QTY to ' . $this->getStockData('qty') . ' and set to SDS to Yes and firstly added to the DXBS - Admin edit Product detail page changed';
                }

                $prevoiusQty = $this->getStockData('original_inventory_qty');
                $qty = $this->getStockData('qty');
                Mage::helper('gearup_sds')->recordHistory($this->getId(), $action, $prevoiusQty, $qty);

            } else if ($this->getStockData('qty') != null) {
                $this->setData('same_day_shipping', 0);
                $nowstatus = $this->getStockData('qty') ? 'In Stock' : 'Out of Stock';
                $nowSds = Mage::helper('gearup_sds')->sdsStatus($this->getData('same_day_shipping'));
                //Mage::helper('gearup_sds')->unassignSDS($this->getId());
                if ($this->getData('stock_item')) {
                    $action = '"' . $this->getStockData('original_inventory_qty')  . ' ' . $previousStockStatus . '; SDS is ' . $previousSds . '", Update QTY to ' . $this->getStockData('qty') . ', "' . $this->getStockData('qty') . ' ' . $nowstatus . '; SDS is  ' . $nowSds . '" - Admin edit Product detail page changed';
                } else {
                    $action = '"New Product", Update QTY to ' . $this->getStockData('qty') . ' and set to SDS to No and firstly added to the DXBS - Admin edit Product detail page changed';
                }
            ...
            Mage::getSingleton('core/session')->unsPsds();
        }
