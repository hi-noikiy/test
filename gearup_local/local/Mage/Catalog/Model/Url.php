...
class Mage_Catalog_Model_Url
{
    ...
    public function getUnusedPathByUrlKey($storeId, $requestPath, $idPath, $urlKey)
    {
        ...
        if ($rewrite && $rewrite->getId()) {
            ...
            $match['prefix'] = ($noSuffixPath ?: $match['prefix']) . '-';
            unset($match['increment']); // don't start counting with a possible number in the url_key
            $match['suffix'] = isset($match['suffix']) ? $match['suffix'] : '';
    ...
    public function refreshProductRewrites($storeId)
    {
        ...
        $excludeProductsdiscontinue = Mage::getStoreConfigFlag('dev/index/discontinue');
        ...
        while ($process == true) {
            ...
            foreach ($products as $product) {
                ...
                if($enableOptimisation && $excludeProductsdiscontinue && $product->getData("discontinued_product")==1) {
                    continue;
                }
