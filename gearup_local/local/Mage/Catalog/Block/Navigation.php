...
class Mage_Catalog_Block_Navigation extends Mage_Core_Block_Template
{
    ...
    public function getCurrenCategoryKey()
    {
        if (!$this->_currentCategoryKey) {
            ...
            if ($category) {
                ...
                $this->_currentCategoryKey = 'p' . $product->getId();
            } else {
        ...
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
		...
		//custom code for navigation starts here--
		$product = Mage::registry('current_product');
        if(!empty($product) && Mage::app()->getFrontController()->getAction()->getFullActionName() == 'catalog_product_view'){
            $catss = $product->getCategoryIds();
            foreach($catss as $cats){
              $cid = Mage::getModel('catalog/category')->load($cats)->parent_id;
            }
            $dat = implode(',',$catss);
            $cmid = $category->getId();

            if(($cmid==$cid) || ($cmid==$dat)){
              $classes[] = 'active';
            }
        } else if ($this->isCategoryActive($category) && Mage::app()->getFrontController()->getAction()->getFullActionName() == 'catalog_category_view') {
              $classes[] = 'active';
        }
		/***custom code for navigation ends up here***/
        ...
        $htmlLi .= ' id="'.$category->getId().'">';
        $html[] = $htmlLi;

        if($level == 0) {
            $html[] = '<span class="cat-head">';
        }
