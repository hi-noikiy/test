...
class Mage_Adminhtml_Block_Catalog_Product_Edit_Action_Attribute extends Mage_Adminhtml_Block_Widget
{
            ...
            $this->setChild('back_button',
                $this->getLayout()->createBlock('adminhtml/widget_button')
                    ->setData(array(
                        ...
                        'onclick'   => 'setLocation(\''.$this->getUrl('*/sds_sds/', array('store'=>$this->getRequest()->getParam('store', 0))).'\')',
                        'class' => 'back'
                    ))
            );
        } else {
    ...
    {
        if ($this->getRequest()->getParam('sds')) {
            return $this->getUrl('*/*/saveattri', array('store'=>Mage::helper('adminhtml/catalog_product_edit_action_attribute')->getSelectedStoreId(), 'sds'=>1));
        } else {
