    ...
    protected function _initTotals()
    {
        ...
        if($taxPercent > 0){
            ...
            $this->_totals['tax'] = new Varien_Object(array(
                'code'  => 'tax',
                'value' => (float)$source->getTaxAmount(),
                'label' => $this->__('VAT Amount ('.$taxPercent.'%)')
            ));
        }
         ...
         * Add cod
         */
        if (Mage::app()->getRequest()->getControllerName() == 'onepage' && Mage::app()->getRequest()->getActionName() == 'success') {
            if (((float)$this->getSource()->getCodFee()) != 0) {
                $this->_totals['cash_on_delivery'] = new Varien_Object(array(
                    'code'  => 'cash_on_delivery',
                    'field'  => 'cash_on_delivery',
                    'value' => $source->getCodFee(),
        ...
        if($paymentMethodCode == 'checkoutapijs' || $paymentMethodCode == 'paypal_standard'){
            $toPay = $this->__('Paid');
        }else{
            $toPay = $this->__('To Pay');
        }
