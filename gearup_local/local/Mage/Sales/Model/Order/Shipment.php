...
class Mage_Sales_Model_Order_Shipment extends Mage_Sales_Model_Abstract
{
    ...
    public function getRefTrack($trackingN)
    {
        $ref = Mage::helper('ffdxshippingbox')->getTrackingRef($trackingN);
        return $ref;
    }

    public function getDestination($order)
    {
        $destination = Mage::helper('ffdxshippingbox')->getDestination($order);
        return $destination;
    }

    public function getDestinationNumber($order,$title)
    {
        $destinationnumber = Mage::getModel('gearup_shippingffdx/destination')->destinationNumber($order->getShippingAddress()->getCountryId(),$title);
        return $destinationnumber;
    }
    public function getTrackingUrl($order,$title)
    {
        $TrackingUrl = Mage::getModel('gearup_shippingffdx/destination')->TrackingUrl($order->getShippingAddress()->getCountryId(),$title);
        return $TrackingUrl;
    }
}
