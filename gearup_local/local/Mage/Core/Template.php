            ...
            if (strpos($includeFilePath, realpath($this->_viewDir)) === 0 || $this->_getAllowSymlinks()) {
                if (Mage::getIsDeveloperMode()) {
                    try {
                        ...
                        $message = sprintf(
                            'ERROR IN %s (%s), TEMPLATE: %s, MESSAGE: %s',
                            $this->getNameInLayout(),
                            get_class($this),
                            $fileName,
                            $e->getMessage()
                        );
                        printf('<div style="border:2px dotted #F00; padding:10px; margin:10px 0; text-align:center; color:#F00;">%s</div>', $message);
                        Mage::log($message, Zend_Log::WARN, null, null, true);
                    }
                }
                else {
                    if (strpos($includeFilePath, realpath($this->_viewDir)) === 0 || $this->_getAllowSymlinks()) {
